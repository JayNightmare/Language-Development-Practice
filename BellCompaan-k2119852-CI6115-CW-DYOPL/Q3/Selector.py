...
class Selector:
    def __init__(self, filter_func=None, sort_key=None, reverse=False, limit=None):
    ...
    @staticmethod
    def top(number, attribute):
        if attribute == TopAttribute.BIGGEST.value:
            return Selector(sort_key=lambda x: x.size, reverse=True, limit=number)
        elif attribute == TopAttribute.SMALLEST.value:
            return Selector(sort_key=lambda x: x.size, reverse=False, limit=number)
        elif attribute == TopAttribute.OLDEST.value:
            return Selector(sort_key=lambda x: x.mtime, reverse=False, limit=number)
        elif attribute == TopAttribute.NEWEST.value:
            return Selector(sort_key=lambda x: x.mtime, reverse=True, limit=number)
        else:
            raise ValueError(f"Invalid top attribute: {attribute}")
